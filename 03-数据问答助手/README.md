...
[查看招聘向项目案例](CASE_STUDY.md)｜[返回作品集首页](../README.md)
...
Streamlit UI (app.py)
        ...
        ├── data_management.py ── 演示数据的受控人工增删与恢复
...
Windows 已配置项目环境时，可以直接双击 `启动项目.bat`。脚本优先使用项目内的 `.venv`，否则使用系统 Python；它只负责启动应用，不会保存或修改 API Key。
...
应用首次进入数据库模式时，会由 `sample_data.csv` 自动生成本地 `sales.db`。`sample_data.csv` 是需要随仓库分发的演示数据，`sales.db` 是本机运行时数据，不提交到 Git。也可以手动重建：

```powershell
python init_db.py
```

未配置 API Key 时应用仍可启动、浏览和人工管理本地演示数据，但自然语言提问会被禁用。
...
1. 填写表单新增一条销售记录。
2. 在表格中选择一条或多条记录，确认后删除。
3. 使用“恢复示例数据”清除本地修改，并重新载入 `sample_data.csv`。

人工管理使用独立的参数化写入接口；AI 生成的 SQL 仍通过只读连接执行，不能新增、修改或删除数据。
...
测试覆盖：提示词、应用服务链路、SQL / Python 执行器、数据质量、数据库白名单、演示数据增删与恢复、图表规则、持久化、配置、错误翻译、示例库初始化、启动脚本和运行日志。

## 演示建议

1. 直接进入数据库模式，展示示例表结构和数据预览。
2. 展开“管理演示数据”，新增一条记录并演示选择删除或恢复初始数据。
3. 提问“各商品销售额的波动情况如何？”，展示多序列折线图和近重合点交互。
4. 展开“分析过程”，说明 SQL、执行结果和最终答案如何对应。
5. 切换上传文件，上传 `sample_data.csv`，展示数据质量概览。
6. 介绍 AI 只读查询和人工受控写入为什么需要分离。
7. 运行离线测试，说明为什么测试不依赖外部模型服务。
...
- 人工数据管理仅面向本地 `sales` 演示表，不是通用数据库后台；生产环境仍需身份认证、角色权限和操作审计。
