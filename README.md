...
- **模块化结构**：AI 服务、安全执行器、审计记录和图表构建器相互独立，便于离线测试与后续替换
...
> 安全说明：程序执行能提高数值计算的可靠性，但 AI 仍可能选错字段或聚合方式，因此“分析过程”会保留生成逻辑和结果预览供核对。AST 允许清单和受限 `exec` 属于学习项目中的分层防护，不等同于进程隔离或操作系统级安全沙箱，不应直接用于执行不受信任用户提交的代码。
